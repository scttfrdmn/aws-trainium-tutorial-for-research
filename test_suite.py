...
def sample_climate_data():
    ...
    data = {
        'date': dates,
        'temperature': 20 + 10 * np.sin(2 * np.pi * np.arange(len(dates)) / 365.25) + np.random.normal(0, 2, len(dates)),
        ...
        'solar_radiation': 200 + 100 * np.sin(2 * np.pi * np.arange(len(dates)) / 365.25) * np.random.random(len(dates))
    }
...
from aws_trainium_tutorial.core import EphemeralMLInstance, CostTracker, NeuronMigrationHelper
...
import pytest
from aws_trainium_tutorial.utils import estimate_training_cost, get_optimal_instance_type
...
import tempfile
import pandas as pd
from aws_trainium_tutorial.examples.domain_specific.climate_science import ClimateDataset
...
class TestBenchmarks:
    ...
    def test_memory_usage_estimation(self):
        ...
        assert large_estimate['estimated_time_hours'] > small_estimate['estimated_time_hours']
...
class TestUtils:
    ...
    def test_configuration_validation(self):
        ...
        assert valid_config['instance_type'] in ['trn1.2xlarge', 'trn1.32xlarge', 'trn2.48xlarge']
...
markers =
    ...
    neuron: marks tests that require Neuron hardware
filterwarnings =
...
deps = 
    ...
    isort>=5.12
commands = 
    black --check aws_trainium_tutorial tests
    flake8 aws_trainium_tutorial tests
    mypy aws_trainium_tutorial
    isort --check-only aws_trainium_tutorial tests
...
deps = 
    ...
    boto3>=1.34.0
commands = 
    pytest tests/test_integration.py -m integration {posargs}
